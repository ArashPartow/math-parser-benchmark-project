  ...
  private:
    ...
    void InitFun() 
    {
      if (!details::value_traits<TValue>::IsInteger())
      {
        ...
        DefineFun( _SL("log"),   MathImpl<TValue, TString>::Log10, 1);
    ...
    void InitOprt() 
    {
      ...
      // Note: Addition is used as a right associative operator
      //       since associativity doesnt matter and right associative
      //       operators yield a RPN that is better suited for optimization.
      DefineOprt( _SL("+"), MathImpl<TValue, TString>::Add, prADD_SUB /*, oaRIGHT*/);
