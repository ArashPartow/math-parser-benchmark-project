    ...
    const TValue& ParseCmdCode() const
    {
      ...
      for (const token_type *pTok = m_vRPN.GetBase(); pTok->Cmd!=cmEND; ++pTok)
      {
        ...
        case  cmVAL_EX: 
              { 
                pVal = &(pTok->Val);
                *(pTok->Stack) = *pVal->ptr * pVal->mul + pVal->fixed; 

                if (pVal->ptr2!=nullptr)
                  *(pTok->Stack2) = *pVal->ptr2 * pVal->mul2 + pVal->fixed2; 
              }
              continue;
        ...
        case  cmFUNC:
              { 
                pFun = &(pTok->Fun);
                (*pFun->ptr)(pTok->Stack, pFun->argc);

                if (pFun->ptr2!=nullptr)
                {
                  (*pFun->ptr2)(pTok->Stack2, pFun->argc2);

                  if (pFun->ptr3!=nullptr)
                    (*pFun->ptr3)(pTok->Stack3, pFun->argc3);
                }
              }
              continue;
