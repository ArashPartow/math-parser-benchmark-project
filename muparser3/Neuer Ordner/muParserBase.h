 ...
 private:
    ...
    const TValue& ParseCmdCode() const
    {
      ...
      for (const token_type *pTok = m_vRPN.GetBase(); pTok->Cmd!=cmEND; ++pTok)
      {
        ...
        {
        case  cmASSIGN: 
              *pTok->Stack = *pTok->Oprt.ptr = *(pTok->Stack+1); 
              continue;
        ...
        case  cmVAL_EX: 
              { 
                m_pVal = &(pTok->Val);
                *(pTok->Stack) = *m_pVal->ptr * m_pVal->mul + m_pVal->fixed; 

                if (m_pVal->ptr2!=nullptr)
                  *(pTok->Stack2) = *m_pVal->ptr2 * m_pVal->mul2 + m_pVal->fixed2; 
              }
              continue;

        case  cmVAR:  *pTok->Stack = *pTok->Val.ptr;   continue;
        case  cmVAL:  *pTok->Stack =  pTok->Val.fixed; continue;
        case  cmFUNC:
              { 
                m_pFun = &(pTok->Fun);
                (*m_pFun->ptr)(pTok->Stack, m_pFun->argc);

                if (m_pFun->ptr2!=nullptr)
                {
                  (*m_pFun->ptr2)(pTok->Stack2, m_pFun->argc2);

                  if (m_pFun->ptr3!=nullptr)
                    (*m_pFun->ptr3)(pTok->Stack3, m_pFun->argc3);
                }
              }
              continue;
  ...
  #define SXO_VAL(TOK) \
          *(TOK)->Stack = *(TOK)->Val.ptr * (TOK)->Val.mul + (TOK)->Val.fixed;       \
          if ((TOK)->Val.ptr2!=nullptr)                                              \
            *(TOK)->Stack2 = *(TOK)->Val.ptr2 * (TOK)->Val.mul2 + (TOK)->Val.fixed2;


  #define SXO_FUN(TOK)                                    \
          {                                               \
            const token_type::SFunDef &fun = (TOK)->Fun;  \
            (*fun.ptr)((TOK)->Stack, fun.argc);           \
            if (fun.ptr2!=nullptr)                        \
            {                                             \
              (*fun.ptr2)((TOK)->Stack2, fun.argc2);      \
              if (fun.ptr3!=nullptr)                      \
                (*fun.ptr3)((TOK)->Stack3, fun.argc3);    \
            }                                             \
          }
    ...
    {    
      return m_pStack[1] = m_pRPN->Val.fixed;
    }

    //---------------------------------------------------------------------------------------------
    const TValue& ParseCmdCode_C2() const
    {    
      return m_pStack[1] = *m_pRPN->Val.ptr * m_pRPN->Val.mul;
    }

    //---------------------------------------------------------------------------------------------
    const TValue& ParseCmdCode_V() const
    {    
      return m_pStack[1] = *m_pRPN->Val.ptr * m_pRPN->Val.mul + m_pRPN->Val.fixed;
    }
    ...
    mutable const typename token_type::SValDef *m_pVal;
    mutable const typename token_type::SFunDef *m_pFun;
    mutable int m_nIdx;
